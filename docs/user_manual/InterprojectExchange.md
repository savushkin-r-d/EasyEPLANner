...
10. [Сохранение обмена сигналами](#10-Сохранение-обмена-сигналами)
...
<p align="justify">Для того, чтобы существующий обмен сигналов был успешно загружен, необходимо, чтобы все связанные проекты имели файл <i>shared.lua</i>. Модуль считывает весь файл <i>shared.lua</i> для текущего открытого (<i>открытого</i>) проекта, и нужные фрагменты из остальных (<i>альтернативных проектов</i>). На рисунке ниже показан пример, как может выглядеть обмен между двумя проектами.</p>
...
1. Список сигналов с их описанием, которые есть в открытом проекте
...
4. Название открытого проекта
5. Строка поиска по сигналам открытого проекта
6. Кнопка настройки фильтрации сигналов в открытом проекте (_описано в следующих разделах_)
...
<p align="justify">Режим обмена (<i>цифра <b>7</b> на картинке</i>), позволяет переключить направление обмена сигналами. Режим определяется относительно открытого проекта. Когда включен режим "<b>Источник >> Приемник</b>" - это значит, что открытый проект (<i>PL-Тест-монитора, на рисунке</i>) является <b>источником (сервером)</b> сигналов, а альтернативный проект является <b>приемником (клиентом)</b> сигналов. Таким образом, сигналы открытого проекта будут записаны в <b>shared_devices</b>, а сигналы альтернативного проекта в <b>remote_gateways</b> (<i>настройка PAC для remote_gateways описана в разделах ниже</i>).</p>

<p align="justify">Режим обмена "<b>Приемник >> Источник</b>" работает в обратном порядке. В этом режиме, открытый проект является приемником сигналов из альтернативного проекта (<b>источника</b>). Сигналы открытого проекта будут записаны в <b>remote_gateways</b>, а альтернативного в <b>shared_devices</b>.
...
1. Выбрать сигнал в списке сигналов открытого проекта
...
<p align="justify">Главными типами сигналов являются сигналы открытого проекта. При связывании <b>AI</b> сигнала открытого проекта с <b>AO</b> сигналом альтернативного проекта, сигнал будет добавлен в группу <b>AI</b>.</p>
...
3. Известен тип сигнала открытого проекта - аналогично пункту <b>1</b>
4. Неизвестны оба типа сигналов - будет показано окно, где надо отметить тип сигнала для открытого проекта (_см. рисунок ниже_)
...
Описание формы:
1. Кнопки выбора типа сигнала открытого (_текущего_) проекта
...
<p align="justify">Уже существующие связи сигналов можно редактировать. Модуль позволяет менять любой сигнал на любой другой и перетаскивать сигналы внутри одной группы. Для редактирования, необходимо выбрать сигналы (<i>пару</i>), которую нужно редактировать (<i>пример ниже</i>). Эта возможность заблокирована, когда включено скрытие уже связанных сигналов.</p>

<p align="justify">Для перетаскивания сигналов вверх-вниз в пределах одной группы нужно использовать сочетание клавиш <b>Shift</b> + <b>Up/Down</b> (<i>стрелочки</i>) для перетаскивания вверх и вниз соответственно. При отключенном режиме отображения группировки сигналов, перетаскивать сигналы по всему списку нельзя т.к группировка по сигналам формируется вне зависимости от того, включено отображение групп или нет.</p>
...
<p align="justify">На рисунке показана выбранная пара сигналов (<i>цифра 1</i>), а также подсвечены сигналы в списках сигналов по проектам (<i>цифра 2</i>). Соответственно, после этого, мы можем менять выбранный сигнал в открытом и альтернативных проектах. Для этого просто кликаем на интересующие сигналы в списках и они будут заменены на новые.</p>

<p align="justify">Учитывайте, что при зеркальной замене во время редактирования (<i>когда появится два одинаковых сигнала в списке</i>), например, когда были перепутаны местами сигналы, произойдет автоматическая замена сигналов между собой, после того как будет произведен клик по сигналу для замены. То есть, первый сигнал меняется пользователем, а второй - автоматически, на основании того, какой сигнал выбрал пользователь. Нет необходимости менять вторую пару сигналов, она поменяется автоматически.</p>

<p align="center"><img src="images/InterProjectExchange/SwapSignals.png" alt="Свап сигналов"></p>

<p align="center"><b>Рисунок</b> - <em>Автоматическая замена сигналов между собой</em></p>
...
<p align="justify">Модуль позволяет настраивать параметры контроллера для межпроектного обмена (см. рисунок ниже). Для настройки учитывается режим, выбранный в главном окне межпроектного обмена. В режиме "<b>Источник</b>" >> "<b>Приемник</b>" настраиваются контроллеры приемников т.к источник является сервером и приемник их считывает по IP-адресу источника. В режиме "<b>Приемник</b>" >> "<b>Источник</b>", открытый проект является приемником сигналов, соответственно, настраивается контроллер открытого проекта, его взаимодействие с источниками сигналов.</p>
...
Описание формы:
1. Список с загруженными для обмена проектами
2. Имя проекта, с которым настраивается контроллер
3. IP-Адрес контроллера (_будет меняться в зависимости от режима открытого проекта_)
4. IP-Адрес эмулятора
5. Кнопки включения/отключения эмуляции
6. Время цикла, мс.
7. Таймаут, мс.
8. Порт
9. Кнопки включения/отключения шлюза
10. Номер станции Modbus (_номер клиента_)
11. Кнопка применения и сохранение изменений
12. Кнопка отмены изменений и закрытия формы

#### Как с этим работать ####

Обмен сигналами у нас идет от по принципу "<b>Источник - Приемник</b>" или "<b>Приемник - Источник</b>". 

#### Номер станции Modbus ####

Номер станции Modbus или (_номер клиента_) должен соответствовать диапазону:

**"Источник >> Приемник"**  
<code>"МСА >> МСА"</code>, <code>"Проект >> МСА"</code> : *220 < station number < 255*;  
<code>"МСА >> Проект"</code>, <code>"Проект >> Проект"</code> : *1 < station number < 220*.

**ВАЖНО**: Если сигналы не заполнены, то будет заблокировано поле с номером станции, чтобы не вводить в заблуждение. 

<p align="justify">Когда селектор открытого проекта стоит в положении "<b>Источник</b>", настраивается контроллер для альтернативного проекта. В поле "<b>IP-Адрес</b>" указывается адрес открытого проекта (<i>автоматически</i>). Эта информация будет записана в <i>shared.lua</i> файл альтернативного проекта.</p>

<p align="justify">При переключении селектора открытого проекта в положение "<b>Приемник</b>", настраивается контроллер для открытого проекта. В поле "<b>IP-Адрес</b>" указывается адрес альтернативного проекта (<i>который выбран из списка</i>). Эта информация будет записана в <i>shared.lua</i> файл открытого проекта.</p>

<p align="justify">Таким образом, когда селектор режима открытого проекта стоит в положении "<b>Источник</b>", то настраиваются параметры чтения сигналов проекта альтернативными проектами, а когда селектор стоит в положении "<b>Приемник</b>", уже наоборот - настраиваются параметры чтения сигналов проекта главным проектом из альтернативных проектов.</p>

## 10 Сохранение обмена сигналами ##

<p align="justify">Для того, чтобы сохранить обмен сигналами, нужно нажать кнопку "<b>Сохранить</b>" в главном окне. Система запустит сохранение и закроет форму. В итоге, будут сгенерированы файлы <i>shared.lua</i> для всех проектов, которые участвовали в обмене сигналами.</p>

<p align="justify">Изменения альтернативных проектов работает так: если обмена с этим проектом не было, то запись будет сделана в начале определения переменной (<i>если это прием сигналов - в начале переменной <b>remote_gateways</b>, а если источник сигналов - в начале <b>shared_devices</b></i>), а если обмен с этим проектом уже был (<i>редактировали сигналы, настройки контроллера</i>), то изменения будут произведены в том месте, где редактировался обмен сигналами (<i>там, где будут найдены строки, отвечающие за обмен с конкретным проектом</i>). Если происходят изменения в открытом проекте - по такому же сценарию.</p>
...
Помимо поиска существуют следующие механизмы настройки (см. рисунок ниже):

1. Фильтрация сигналов по их типам
2. Скрывание уже связанных сигналов из списка сигналов
3. Отключение контроля логичности связываемых сигналов (_для проброса сигналов между проектами_)
...
2. Список типов сигналов для открытого проекта
3. Настройка общих параметров обмена сигналами
4. Список типов сигналов для альтернативного проекта
5. Кнопка применения изменений
6. Кнопка отмены изменений

Описание общих параметров обмена сигналами:
1. Фильтрация сигналов по их типам включает отображение в списке уже связанных сигналов, их групп. Распределение по группам идет по сигналам текущего редактируемого (открытого) проекта. Этот функционал делает более наглядным распределение сигналов, сигналы не будут казаться сваленными в одну кучу.
2. Скрывание уже связанных сигналов позволит убрать из списков сигналов те сигналы, которые уже связаны, тем самым уменьшается список сигналов и настраивать обмен легче. Список автоматически обновляется при добавлении связи или при её удалении. Редактирование связи, когда включена эта функция - запрещено. Редактирование будет активно, если отключить этот функционал с помощью галочки.
3. Отключение контроля пар сигналов позволяет пробросить сигналы из одного проекта в другой без необходимости создания дополнительных пар сигналов. То есть, можно обменяться парами **DO-DO**, **DI-DI** и др. Аналоговые сигналы с дискретными по-прежнему нельзя связывать, это ограничение осталось. Пример работы этого функционала ниже на картинке.

<p align="center"><img src="images/InterProjectExchange/DisableCheckPairsSignals.png" alt="Пример настройки проброса сигналов"></p>

<p align="center"><b>Рисунок</b> - <em>Пример настройки проброса сигналов</em></p>
...
4. "Ошибка сохранения промежуточных данных" - при настройке PAC (_контроллера_), не смогли сохраниться данные в одном из трех случаев:
    - изменение режима открытого проекта
...
10. "Ошибка связки сигналов. Попытка связать уже связанный(-е) сигнал(-ы)" - один или оба сигналы уже связаны (_имеют связь такую же, или с другим сигналом_). Блокировка от ошибочной привязки.
...
4. "Проверьте номера станций открытого проекта в режиме "Источник", дублирование номеров" - дублируются номера станций открытого проекта (_который открыт в EPLAN_).
...
<p align = "justify">В этом разделе расписано, что надо переделать в старом описании <i>shared.lua</i>, что бы его можно было редактировать через этот модуль. Пример основан на тестовых проектах, открытый проект - <code>"PL-Тест-монитора"</code>, альтернативный проект - <code>"PL-Тест-монитора-новый"</code>. Обмен происходит в режиме <code>"Источник >> Приемник"</code>.</p>

<p align = "justify">Открытый и альтернативный проект ничем не отличаются по своей сути, кроме того, что открытый - всегда открыт в EPLAN, а альтернативный - обменивается с главным проектом. Любой открытый проект в EPLAN будет являться главным, а все проекты, с которыми он обменивается - альтернативными.</p>

Необходимые изменения в __shared_devices__:
```diff
shared_devices =                    
{                               
...
Как видно из кода, мы добавили строку `projectName = "PL-Тест-монитора-новый"` в _shared_lua_ открытого проекта (который открыт в Eplan). В этом коде, открытый проект отправляет свои сигналы проекту, имя которого мы добавили.

Необходимые изменения в __remote_gateways__ :
```diff
remote_gateways =
{
-   ['PL-TEST-MONITOR'] =
