...
Модуль позволяет настраивать параметры контроллера для межпроектного обмена (см. рисунок ниже). Для настройки учитывается режим, выбранный в главном окне межпроектного обмена. В режиме "Источник" >> "Приемник" настраиваются контроллеры приемников т.к источник является сервером и приемник их считывает по IP-адресу источника. В режиме "Приемник" >> "Источник", открытый проект является приемником сигналов, соответственно, настраивается контроллер открытого проекта, его взаимодействие с источниками сигналов.
