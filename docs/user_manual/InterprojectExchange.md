...
4. "Проверьте номера станций главного проекта в режиме "Источник", дублирование номеров" - дублируются номера станций главного проекта (который открыт в EPLAN).
...
## 14 Более краткое объяснение преобразования старой функциональности в новую  ##

Контроллер одновременно является **сервером** и **клиентом**. Например, в проекте `T1-Проект` настраивается связь с каким-то проектом `T2-Проект`.

### 14.1 Преобразование сервера ###
Когда контроллер в режиме сервера, то он может передать сигналы клиентам, которые к нему подключились. Идентификация клиента идет по номеру. Номером является индекс массива. Сервера прописываются в **shared_devices**.

Номера клиентов на сервере **не** могут пересекаться, и система это контролирует. Внизу представлен фрагмент описания отправки трех сигналов с сервера клиенту с номером 2.

```LUA
shared_devices =
{
    [2] = -- 2 это номер клиента.
    {
        DI =
        {
        __DI2,
...
В старом описании название проекта нигде не фигурировало и писалось латиницей. В новом же описании это название нужно изменить на корректное название проекта. Название проекта нужно брать также, как и при преобразовании сервера (*из папки с проектами*). Это название используется при идентификации клиента (*чтобы проект мог найти клиенты, которые к нему обращаются*), следовательно, это название будет фигурировать в файлах, которые не являются файлами проекта с названием клиента. Например, настройка связи проекта `T1-Проект` с проектом `T2-Проект`. В файле **shared.lua** проекта `T2-Проект` будет указано имя сервера, а именно `T1-Проект`. Пример ниже.
