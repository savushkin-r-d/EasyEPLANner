...
В этом разделе расписано, что надо переделать в старом описании shared.lua, что бы его можно было редактировать через этот модуль.

Фрагмент кода из главного проекта:
...
Фрагмент кода из альтернативного проекта:
...
Как видно из этих двух кусков кода, когда мы указываем проект в shared_devices, мы также должны указывать строку <code>projectName = "PL-Тест-монитора-новый",</code>, для того, что бы при считывании shared_devices мы знали, какому проекту эти сигналы отправляются.

Также, когда мы указываем настройки контроллера в remote_gateways, ключом таблицы является текущее название проекта <code>['PL-Тест-монитора']</code>, из которого читаются сигналы. Так мы можем знать, в каком проекте надо сигналы прочитать.

Это все изменения, которые позволят читать межконтроллерный обмен, сделанный до появления этого функционала.