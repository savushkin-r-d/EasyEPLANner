...
Номера клиентов на сервере **не** могут пересекаться, и система это контролирует. Внизу представлен фрагмент старого описания отправки трех сигналов с сервера клиенту с номером 2 (_номер станции_).
...
В режиме _И_сточник >> Приемник__ для такой настройки меняется **shared_devices** в редактируемом проекте и **remote_gateways** в проектах-клиентах. В обратном режиме наоборот.
