...
<p align="justify">Режим обмена (<i>цифра <b>7</b> на картинке</i>), позволяет переключить направление обмена сигналами. Режим определяется относительно главного проекта. Когда включен режим "<b>Источник >> Приемник</b>" - это значит, что открытый проект (<i>PL-Тест-монитора, на рисунке</i>) является <b>источником (сервером)</b> сигналов, а альтернативный проект является <b>приемником (клиентом)</b> сигналов. Таким образом, сигналы главного проекта будут записаны в <b>shared_devices</b>, а сигналы альтернативного проекта в <b>remote_gateways</b> (<i>настройка PAC для remote_gateways описана в разделах ниже</i>).</p>

<p align="justify">Режим обмена "<b>Приемник >> Источник</b>" работает в обратном порядке. В этом режиме, открытый проект является приемником сигналов из альтернативного проекта (<b>источника</b>). Сигналы главного проекта будут записаны в <b>remote_gateways</b>, а альтернативного в <b>shared_devices</b>.
...
1. Фильтрация сигналов по их типам включает отображение в списке уже связанных сигналов, их групп. Распределение по группам идет по сигналам текущего редактируемого (главного) проекта. Этот функционал делает более наглядным распределение сигналов, сигналы не будут казаться сваленными в одну кучу.
...
Как видно из кода, мы добавили строку `projectName = "PL-Тест-монитора-новый"` в _shared_lua_ главного проекта (который открыт в Eplan). В этом коде, открытый проект отправляет свои сигналы проекту, имя которого мы добавили.

Рассмотрим фрагменты кода, когда мы принимаем сигналы из главногое проекта в альтернативном (который не открыт в Eplan, но открытый проект взаимодействует с этим проектом, как с альтернативным).
