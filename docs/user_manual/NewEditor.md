...
Для того, чтобы использовать новое дерево объектов (<em>оно открывается поверх старого, заменяя его элементы, таким образом, нельзя работать сразу в двух деревьях</em>). Необходимо в меню "<b>EPlaner</b>" выбрать пункт "<strong>Новый редактор объектов</strong>" (<em>см. картину ниже</em>).
...
<p align="justify">Неопознанный объект может им и оставаться. Но над ним нельзя выполнять никаких действий, кроме как: "<b>Удалить</b>", "<b>Вырезать</b>". Если не переносить базовый объект в другие объекты, то он будет сохраняться как неопознанный объект.</p>
