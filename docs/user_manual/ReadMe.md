...
+ [Перечень устройств](#62-Перечень-устройств)
    ...
    + [R. ПИД-регулятор](#6222-R-ПИД-регулятор)
...
+ [Работа с ПИД-регулятором R](#65-Работа-с-ПИД-регулятором-R)
...
+ [Использование функциональности базового объекта и базовой операции](#73-Использование-функциональности-базового-объекта-и-базовой-операции)
    ...
    + [Использование системных параметров в базовом объекте](#735-Использование-системных-параметров-в-базовом-объекте)
    + [Список базовых объектов, операций, параметров, шагов и оборудования](#736-Список-базовых-объектов-операций-параметров-шагов-и-оборудования)
    + [Рекомендации по использованию ОУ (обозначений устройств) в объектах](#737-Рекомендации-по-использованию-ОУ-обозначений-устройств-в-объектах)
...
#### 6.2.22 R. ПИД-регулятор ####
Обозначение: **R**
Каналы ввода/вывода: отсутствуют, виртуальное устройство. Только внутренние параметры.
Пример: **OBJ1+TANK1-R1**. Проект OBJ1, Танк №1, ПИД-регулятор танка №1.
...
* ___R___
    ...
    * __OUT_VALUE__ - Устройство, задающее выходную величину для ПИД-регулятора (исполнительное устройство или указывается другой ПИД, если нужен каскадный ПИД_).
...
Изделия не проверяются в устройствах следующих типов: __VC__, __M__ (*кроме **M_ATV***), __DI__, __DO__, __AI__, __AO__, __R__.
...
### 6.5 Работа с ПИД-регулятором R ###

ПИД-регулятор **R** используется как обычное устройство на функциональной схеме автоматизации. У него есть определенный перечень параметров и дополнительных свойств, узнать который можно [здесь](#632-Назначение-дополнительных-полей).

Базовые свойства ПИД-регулятора, которые записываются в **Доп. поле [3]** заданы стандартными значениями, которые описаны в перечне дополнительных свойств и параметров. Если нужно переопределить стандартное значение, то необходимо в **Доп. поле [3]** записать только переопределяемое значение (_см. рисунок ниже_).
...
Использование (_запуск_) ПИД-регулятора осуществляется через использование (_см. рисунок ниже_) его в шагах операции. ПИД-регулятор добавляется во вкладку шага "_Устройства_" в группе во вкладке "__Устройства__", а в поле "__Производительность__" указывается его задание (_параметр или сигнал AI_).

<p align="center"><img src="images/RunPID.jpg"></p>
...
#### 7.3.5 Использование системных параметров в базовом объекте ####
...
#### 7.3.6 Список базовых объектов, операций, параметров, шагов и оборудования ####
...
#### 7.3.7 Рекомендации по использованию ОУ (_обозначений устройств_) в объектах ####
