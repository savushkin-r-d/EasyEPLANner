...
13. **Перейти в шаг по условию**. Специальное действие - переход к шагу по выполнении условия: включение и выключение заданных устройств. Так как устройства могут включаться/выключаться долго, анализируется сигнал обратной связи и по его наличию определяется факт включения/выключения. Операторы для проверки устройств можно выбрать в соответствующем пункте, представляют собой 3 оператора(для включения устройств - между условиями - для выключения устройств). Номер шага указывается в действии "Шаг", при этом номер шага должен быть больше нуля, в ином случае он сбрасывается.
