...
Настройки:
1. Для того, что бы изменить адрес главной страницы, которая будет открываться, если не найдено описание объекта, нужно открыть файл конфигурации <code>configuration.ini</code>, найти секцию <code>helpSystem</code> и в ключе <code>mainAddress</code> нужно указать адрес, который будет открываться.
