...
* ___HLA_VIRT___, ___HLA_IOLINK___
    * __SIGNALS_SEUQENCE__ - последовательность сигналов (лампочек) и сирены на сигнальной колонне (прим., SIGNALS_SEUQENCE='ABGYR'). **A** - сирена, **B** - синяя лампа, **G** - зеленая лампа, **Y** - желтая лампа, **R** - красная лампа. Правила: без дублирования символов, длина последовательности от **1** до **5** символов.
...
Изделия не проверяются в устройствах следующих типов: __VC__ (*кроме **VC_IOLINK***), __M__ (*кроме **M_ATV, M_ATV_LINEAR***), __DI__, __DO__, __AI__, __AO__, __ПИД-регулятор__, __CAM__.
...
11. **Перейти в шаг по условию**. Специальное действие - переход к шагу по выполнении условия: включение и выключение заданных устройств (логическое **И**). Так как устройства могут включаться/выключаться долго, анализируется сигнал обратной связи и по его наличию определяется факт включения/выключения.
...
11. Перейти в шаг по условию - **V**, **GS**, **DI**, **DO**.
