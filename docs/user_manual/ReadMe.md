...
7. **Мойка (DI, DO, устройства)**. Служит для упрощения типовой логики работы. Устройства, которые находятся в группе *"устройства"* включаются (активируются) при активном (хотя бы одном) сигнале из группы *"DI"*. При этом выдается сигналы для группы *"DO"*. Если нет ни одного сигнала из группы *"DI"*, то устройства в группе *"устройства"* отключаются.
