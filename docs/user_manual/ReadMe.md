...
**Примечание**: для IO-Link модуля Phoenix Contact дополнительно после комментария к сигналу (прим., _Пуск_, как на рисунке ниже) через символ новой строки указывается тип канала. Тип каналов может быть: 
1. IO-Link (для каналов AO, AI);
2. DI (для каналов DI); 
3. DO (для каналов DO).
