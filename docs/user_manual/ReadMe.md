...
Описание нового окна редактора, которое будет являться заменой текущего окна редактора, представлено [тут](NewEditor.md). Новый редактор модифицирует текущее дерево с оглядкой на модель рецептурного производства **S-88**.
