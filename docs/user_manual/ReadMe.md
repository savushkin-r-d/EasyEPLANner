...
* ___C_THLD___ (Пороговый регулятор)
    * ___P_is_reverse__ - Выход обратного дейсвтвие (1 - true, 0 - false): задает вектор изменения измеряемой величины. 
    * __P_DELTA__ - Дельта срабатывания.
    * __P_ON_TIME__ - Задержка включения.
    * __P_OFF_TIME__ - Задержка выключения.
...
* ___Регулятор___ (_вариации с символом C_)
    * __IN_VALUE__ - Устройство, задающее входную величину для регулятора (датчик, сигнал AI).
    * __OUT_VALUE__ - Устройство, использующее выходную величину регулятора (исполнительное устройство или другой регулятор, если необходимо каскадное регулирование).
...
Помимо описанных выше действий, шаги включают в себя параметр времени и номер следующего шага для безусловного перехода, посел того как операция была завершена. В поле __"Время (параметр)"__ записывается __номер__ параметра, а в самом параметре устанавливается значение времени в ***миллисекундах (мс)***.   
