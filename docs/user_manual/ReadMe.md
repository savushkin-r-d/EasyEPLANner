...
#### Работа с деревом рецептов ####

+ [Описание редактора рецептов](#14-Описание-редактора-рецептов)
...
- __*main.devices.lua*__ - _устарел_, описывает список всех устройств проекта. В файле переменным Lua присваиваются устройства из проекта, с которыми в последствии будет работать ПЛК. Файл не генерируется, если в проекте его не было т.к в новых версиях управляющей программы он не используется;
...
9. Группы DI -> DO - **DI**, **GS**, **DO**, **SB**, **HL**;
10. Группы инвертированный DI -> DO - **DI**, **GS**, **DO**, **SB**, **HL**;
11. Группы AI -> AO - **AI**, **AO**, **M**.
...
### 14 Описание редактора рецептов ###

Вызвать редактор можно через меню EasyEplanner->Создание списка рецептов. Все объекты представляются в иерархической древовидной структуре.

<p align="center"><img src="images/recipes_form.png"></p>

<p align="center"><b>Рисунок</b> - <i>Окно редактора рецептов</i></p>
...
2. Развернуть/свернуть дерево объектов к 2 уровню.
3. Развернуть/свернуть дерево объектов к 3 уровню.
4. Создать новый пустой рецепт (**Insert**).
5. Удалить выделенный объект (**Del**) - возможность удалить весь рецепт или параметр в рецепте.
6. Копировать выделенный объект(**Ctrl + C**) - возможность скопировать весь рецепт или параметр в рецепте.
7. Вставить скопированный объект в выделенное место как новый (**Ctrl + V**) - возможность вставить весь рецепт или параметр в рецепте.
8. Вставить скопированный объект в выделенной место с заменой выделенного объекта (**Ctrl + B**) - возможность заменить весь рецепт или параметр в рецепте.

Рецепт имеет имя (например, **"Рецепт Тестовая укладка"**) и список применяемых в нем параметров. Каждый параметр представлен именем технологического объекта и параметром данного объекта, к которому будет применено заданное значение по умолчанию (Объект - **"Узел делителя объектов №1"**, параметр - **"Необходимое количество продукта, ед."**, значение по умолчанию - **"81"** ). 

Также для работы с деревом используются клавиши клавиатуры:
...
Для того, чтобы добавить параметр в рецепт, необходимо вызвать окно со списком параметров объектов проекта и выбрать параметры из данного списка при помощи установки галочки напротив соответствующего параметра (снятие галочки удаляет выбранный параметр из рецепта). Данное окно можно вызвать через меню **EasyEplanner->Устройства, параметры объектов**.

<p align="center"><img src="images/add_recipes_param.png"></p>

<p align="center"><b>Рисунок</b> - <i>Выбор параметров из списка доступных</i></p>

После добавления в список нового параметра можно задать ему значение, которое будет использовано по умолчанию при применении рецепта.