...
**Примечание**: 
1. Для IO-Link модуля Phoenix Contact дополнительно после комментария к сигналу (прим., _Пуск_, как на рисунке ниже) через символ новой строки указывается тип канала. Тип каналов может быть: 
    * IO-Link (для каналов AO, AI);
    * DI (для каналов DI); 
    * DO (для каналов DO).
