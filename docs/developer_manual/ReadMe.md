...
2. [Файловая структура](#2-Файловая-структура)
    ...
    + [2.5 Файлы проекта EasyEPlanner](#25-Файлы-проекта-EasyEPlanner)
        ...
        + [2.5.11 Каталог RManager](#2511-Каталог-RManager)
        + [2.5.12 Каталог StaticHelper](#2512-Каталог-StaticHelper)
        + [2.5.13 Каталог TechObject](#2513-Каталог-TechObject)
...
7. sys_io_modules.lua - скрипт с описанием модулей-ввода вывода, которые поддерживаются надстройкой.
8. sys_io_nodes.lua - скрипт с описанием узлов ввода-вывода, которые поддерживаются надстройкой.
9. sys_iolink_devices.lua - скрипт с описанием поддерживаемых IO-Link устройств.
10. sys_restriction.lua - скрипт читающий описание ограничений для технологических объектов (*всегда запускается после sys.lua*).
11. sys_shared_initializer.lua - скрипт читающий межпроектный обмен из файла *main.shared.lua* проекта.
...
#### 2.5.11 каталог RManager ####

Каталог содержит в себе древовидную структуру, описывающую иерархию задания рецепта, которая отображается в редакторе рецептов. Дерево в редакторе отображается при помощи `TreeListView` , наши элементы наследуются от `TreeViewItem` . При выгрузке проекта описание сохраняется в файл `main.recipes.lua`, который при загрузке разбирается скриптами, хранящимися в  `sys_recipes.lua`.

В текущей реализации, чтобы использовать рецепты в проекте (применять, задавать новые, копировать и т.д.), необходимо в дерево технологических объектов добавить объект **"Объект для хранения рецепта"**. Этот объект остается без операций и параметров, работа с ним осуществляется с помощью скриптов, описанных в файле `par_work.lua`.

Редактор рецептов отображает эту модель, и работает с ней.

1. IRecipesManager.cs - интерфейс менеджера рецептов.
2. RecipesManger.cs - главный класс работы с рецептами. Вершина дерева рецептов, осуществляет загрузку/сохранение дерева, хранит список рецептов, позволяет с ними взаимодействовать (добавлять, удалять, перемещать и т.д.).
3. Recipe.cs - класс, который хранит и работает со списком параметров, использующихся в конкретном рецепте.
4. RParam.cs - класс, описывающий параметр рецепта. Это описание включает в себя номер технологического объекта и номер его параметра, который мы хотим в дальнейшем использовать, а также значение этого параметра по умолчанию в конкретном рецепте.

#### 2.5.12 каталог StaticHelper ####
