...
+ [Нумерация устройств](#51-Нумерация-устройств)
    + [Площадка](#511-Площадка)
    + [Проект](#512-Проект)
    + [Технологический объект](#513-Технологический-объект)
    + [Пользовательский объект](#514-Пользовательский-объект)
    + [Устройство](#515-Устройство)
...
- Имя площадки задается символами латинского алфавита с возможностью номерного обозначения площадки (прим., O1, BR1, P1, S1 и др.);
- Имя проекта задается символами кириллического алфавита с возможностью номерного обозначения (прим., МСА1, Ряженка, Рассыпчатый, Гребенка-десертов и др.);
- Знаки пробела используемые в имени проекта должны быть заменены на знак минус.

**Примеры** правильного имени проекта: "BR1-Хуторки", "O1-Танки-смеси", "P1-Плавленые-сыры".
...
В файле напротив ключа __folder_path__ в кавычках указывается путь к каталогу проектов. Например: _folder_path="**P:\PTUSA_projects\projects**"_
...
При множественной привязке, в функциональный текст будет заноситься лишь обозначение устройств (_ОУ_) друг за другом (_прим., "+V1 +V2 +V3"_), причем можно задавать её как в строку (_задается изначально_), так и в столбец (_необходимо расставить символы переноса строки между ОУ_).
...
Пример корректного комментария: **"Подача в линию CR2"**.
...
Обозначение: **Y**, **DEV_VTUG** - старые проекты (_до Eplan 2.8_).  
Каналы ввода/вывода: **AO** – *1*. (AOAI)
Пример: **MCA3+CAB1-Y1**. Проект МСА3, шкаф №1, устройство пневмоостров Festo.
